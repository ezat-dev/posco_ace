...
<body>
  <div class="container no-select">
    <div class="header">고진공 밸브</div>

    <div class="controls" style="margin-top:8px;">
      <button class="btn ctrl-btn" data-tag="vacuum-valve-off">OFF</button>
	  <button class="btn ctrl-btn" data-tag="vacuum-valve-on">ON</button>
...
$.ajax({
   ...
   success: function(res) {
       if (res.status === "OK") {
           ...
           const onBtn = document.querySelector("[data-tag='vacuum-valve-on']");
           if (isOn) {
...
$.ajax({
   url: "/posco/monitoring/read/bit",
   type: "get",
   data: { tagName: "vacuum-valve-lamp-off" },
   success: function(res) {
       if (res.status === "OK") {
           const isOff = res.value === true;

           const offBtn = document.querySelector("[data-tag='vacuum-valve-off']");
           if (isOff) {
