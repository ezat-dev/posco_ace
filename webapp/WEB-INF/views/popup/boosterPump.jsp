...
<body>
  <div class="container no-select">
    <div class="header">저진공 보조 펌프</div>

    <div class="controls" style="margin-top:8px;">
      <button class="btn ctrl-btn" data-tag="booster-pump-off">OFF</button>
	  <button class="btn ctrl-btn" data-tag="booster-pump-on">ON</button>
...
$.ajax({
   ...
   success: function(res) {
       if (res.status === "OK") {
           ...
           const onBtn = document.querySelector("[data-tag='booster-pump-on']");
           if (isOn) {
...
$.ajax({
   url: "/posco/monitoring/read/bit",
   type: "get",
   data: { tagName: "booster-pump-lamp-off" },
   success: function(res) {
       if (res.status === "OK") {
           const isOff = res.value === true;

           const offBtn = document.querySelector("[data-tag='booster-pump-off']");
           if (isOff) {
